...
class GeneralAction {
    ...
    return async (dispatch) => {
      try {
        const { data, status } = await api.get(
          `${process.env.REACT_APP_API_URL}/${this.subURL}?page=${page}&size=${size}&name=${name}`,
        )
  ...
  getOneData(id) {
    return async (dispatch) => {
      try {
        const { data, status } = await api.get(`${process.env.REACT_APP_API_URL}/${this.subURL}/${id}`)

        if (status === 200 && data) {
          dispatch({ type: this.oneConst, payload: { ...data, isReady: true } })
        } else {
          dispatch({ type: this.errorConst, payload: { error: `Get ${this.name} Error` } })
        }
      } catch (error) {
  ...
  editOne(id, payload) {
    return async (dispatch) => {
      try {
        const { data, status } = await api.put(`${process.env.REACT_APP_API_URL}/${this.subURL}/${id}`, payload)

        if (status === 200 && data) {
          dispatch({ type: this.updateConst, payload: { ...data, isReady: false } })
        } else {
          dispatch({ type: this.errorConst, payload: { error: `Edit ${this.name} Error` } })
        }
      } catch (error) {
        dispatch({ type: this.errorConst, payload: { error: error?.message } })
      }
  ...
  deleteOne(id) {
    return async (dispatch) => {
      try {
        const { data, status } = await api.delete(`${process.env.REACT_APP_API_URL}/${this.subURL}/${id}`)

        if (status === 204 && data) {
          dispatch({ type: this.deleteConst, payload: { ...data, isReady: false } })
        } else {
          dispatch({ type: this.errorConst, payload: { error: `Delete ${this.name} Error` } })
        }
      } catch (error) {
        dispatch({ type: this.errorConst, payload: { error: error?.message } })
      }
